...
static const union cptr_el3 CPTR_EL3_SAFEVAL =
{0
};



static inline union cptr_el3 read_cptr_el3( void )
{
    union cptr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c1_c1_2"
...
static inline void unsafe_write_cptr_el3( union cptr_el3 val )
{
    __asm(
        "MSR s3_6_c1_c1_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cptr_el3( ... ) \
    do \
    { \
        union cptr_el3 tmp = CPTR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cptr_el3(tmp); \
    } while (0)



#define read_modify_write_cptr_el3( ... ) \
    do \
    { \
        union cptr_el3 tmp = read_cptr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cptr_el3(tmp); \
    } while (0)
