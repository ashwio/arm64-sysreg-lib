...
static const union sder32_el3 SDER32_EL3_SAFEVAL =
{0
};



static inline union sder32_el3 read_sder32_el3( void )
{
...
static inline void unsafe_write_sder32_el3( union sder32_el3 val )
{
    __asm(
        "MSR s3_6_c1_c1_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_sder32_el3( ... ) \
    do \
    { \
        union sder32_el3 tmp = SDER32_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sder32_el3(tmp); \
    } while (0)



#define read_modify_write_sder32_el3( ... ) \
    do \
    { \
        union sder32_el3 tmp = read_sder32_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sder32_el3(tmp); \
    } while (0)
