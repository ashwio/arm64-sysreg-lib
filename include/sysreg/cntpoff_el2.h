...
union cntpoff_el2
{
    ...
    __extension__ struct {
        u64 Physical_offset : 64;
    };
};



static const union cntpoff_el2 CNTPOFF_EL2_SAFEVAL =
{0
};



static inline union cntpoff_el2 read_cntpoff_el2( void )
{
...
static inline void unsafe_write_cntpoff_el2( union cntpoff_el2 val )
{
    __asm(
        "MSR s3_4_c14_c0_6, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cntpoff_el2( ... ) \
    do \
    { \
        union cntpoff_el2 tmp = CNTPOFF_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntpoff_el2(tmp); \
    } while (0)



#define read_modify_write_cntpoff_el2( ... ) \
    do \
    { \
        union cntpoff_el2 tmp = read_cntpoff_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntpoff_el2(tmp); \
    } while (0)
