...
union far_el2
{
    ...
    __extension__ struct {
        u64 Faulting_Virtual_Address_for_synchronous_exceptions_taken_to_EL2 : 64;
    };
};



static const union far_el2 FAR_EL2_SAFEVAL =
{0
};



static inline union far_el2 read_far_el2( void )
{
    union far_el2 tmp;
    __asm(
        "MRS %0, s3_4_c6_c0_0"
...
static inline void unsafe_write_far_el2( union far_el2 val )
{
    __asm(
        "MSR s3_4_c6_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_far_el2( ... ) \
    do \
    { \
        union far_el2 tmp = FAR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_far_el2(tmp); \
    } while (0)



#define read_modify_write_far_el2( ... ) \
    do \
    { \
        union far_el2 tmp = read_far_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_far_el2(tmp); \
    } while (0)
