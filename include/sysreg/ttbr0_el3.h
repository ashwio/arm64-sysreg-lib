...
static const union ttbr0_el3 TTBR0_EL3_SAFEVAL =
{0
};



static inline union ttbr0_el3 read_ttbr0_el3( void )
{
    union ttbr0_el3 tmp;
    __asm(
        "MRS %0, s3_6_c2_c0_0"
...
static inline void unsafe_write_ttbr0_el3( union ttbr0_el3 val )
{
    __asm(
        "MSR s3_6_c2_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_ttbr0_el3( ... ) \
    do \
    { \
        union ttbr0_el3 tmp = TTBR0_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr0_el3(tmp); \
    } while (0)



#define read_modify_write_ttbr0_el3( ... ) \
    do \
    { \
        union ttbr0_el3 tmp = read_ttbr0_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr0_el3(tmp); \
    } while (0)
