...
static const union sder32_el2 SDER32_EL2_SAFEVAL =
{0
};



static inline union sder32_el2 read_sder32_el2( void )
{
...
static inline void unsafe_write_sder32_el2( union sder32_el2 val )
{
    __asm(
        "MSR s3_4_c1_c3_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_sder32_el2( ... ) \
    do \
    { \
        union sder32_el2 tmp = SDER32_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sder32_el2(tmp); \
    } while (0)



#define read_modify_write_sder32_el2( ... ) \
    do \
    { \
        union sder32_el2 tmp = read_sder32_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sder32_el2(tmp); \
    } while (0)
