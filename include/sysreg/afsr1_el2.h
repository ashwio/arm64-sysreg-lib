...
static const union afsr1_el2 AFSR1_EL2_SAFEVAL =
{0
};



static inline union afsr1_el2 read_afsr1_el2( void )
{
    union afsr1_el2 tmp;
    __asm(
        "MRS %0, s3_4_c5_c1_1"
...
static inline void unsafe_write_afsr1_el2( union afsr1_el2 val )
{
    __asm(
        "MSR s3_4_c5_c1_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_afsr1_el2( ... ) \
    do \
    { \
        union afsr1_el2 tmp = AFSR1_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_afsr1_el2(tmp); \
    } while (0)



#define read_modify_write_afsr1_el2( ... ) \
    do \
    { \
        union afsr1_el2 tmp = read_afsr1_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_afsr1_el2(tmp); \
    } while (0)
