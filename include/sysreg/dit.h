...
static const union dit DIT_SAFEVAL =
{0
};



static inline union dit read_dit( void )
{
    union dit tmp;
    __asm(
        "MRS %0, s3_3_c4_c2_5"
...
static inline void unsafe_write_dit( union dit val )
{
    __asm(
        "MSR s3_3_c4_c2_5, %0"
        : /**/ : "r" (val._)
...
#define safe_write_dit( ... ) \
    do \
    { \
        union dit tmp = DIT_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dit(tmp); \
    } while (0)



#define read_modify_write_dit( ... ) \
    do \
    { \
        union dit tmp = read_dit(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dit(tmp); \
    } while (0)
