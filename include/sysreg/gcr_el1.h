...
static const union gcr_el1 GCR_EL1_SAFEVAL =
{0
};



static inline union gcr_el1 read_gcr_el1( void )
{
    union gcr_el1 tmp;
    __asm(
        "MRS %0, s3_0_c1_c0_6"
...
static inline void unsafe_write_gcr_el1( union gcr_el1 val )
{
    __asm(
        "MSR s3_0_c1_c0_6, %0"
        : /**/ : "r" (val._)
...
#define safe_write_gcr_el1( ... ) \
    do \
    { \
        union gcr_el1 tmp = GCR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_gcr_el1(tmp); \
    } while (0)



#define read_modify_write_gcr_el1( ... ) \
    do \
    { \
        union gcr_el1 tmp = read_gcr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_gcr_el1(tmp); \
    } while (0)
