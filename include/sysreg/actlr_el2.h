...
static const union actlr_el2 ACTLR_EL2_SAFEVAL =
{0
};



static inline union actlr_el2 read_actlr_el2( void )
{
    union actlr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c1_c0_1"
...
static inline void unsafe_write_actlr_el2( union actlr_el2 val )
{
    __asm(
        "MSR s3_4_c1_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_actlr_el2( ... ) \
    do \
    { \
        union actlr_el2 tmp = ACTLR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_actlr_el2(tmp); \
    } while (0)



#define read_modify_write_actlr_el2( ... ) \
    do \
    { \
        union actlr_el2 tmp = read_actlr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_actlr_el2(tmp); \
    } while (0)
