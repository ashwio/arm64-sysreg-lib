...
union scxtnum_el2
{
    ...
    __extension__ struct {
        u64 Software_Context_Number : 64;
    };
};



static const union scxtnum_el2 SCXTNUM_EL2_SAFEVAL =
{0
};



static inline union scxtnum_el2 read_scxtnum_el2( void )
{
...
static inline void unsafe_write_scxtnum_el2( union scxtnum_el2 val )
{
    __asm(
        "MSR s3_4_c13_c0_7, %0"
        : /**/ : "r" (val._)
...
#define safe_write_scxtnum_el2( ... ) \
    do \
    { \
        union scxtnum_el2 tmp = SCXTNUM_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_scxtnum_el2(tmp); \
    } while (0)



#define read_modify_write_scxtnum_el2( ... ) \
    do \
    { \
        union scxtnum_el2 tmp = read_scxtnum_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_scxtnum_el2(tmp); \
    } while (0)
