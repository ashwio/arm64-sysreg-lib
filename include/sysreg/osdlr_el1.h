...
static const union osdlr_el1 OSDLR_EL1_SAFEVAL =
{0
};



static inline union osdlr_el1 read_osdlr_el1( void )
{
    union osdlr_el1 tmp;
    __asm(
        "MRS %0, s2_0_c1_c3_4"
...
static inline void unsafe_write_osdlr_el1( union osdlr_el1 val )
{
    __asm(
        "MSR s2_0_c1_c3_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_osdlr_el1( ... ) \
    do \
    { \
        union osdlr_el1 tmp = OSDLR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_osdlr_el1(tmp); \
    } while (0)



#define read_modify_write_osdlr_el1( ... ) \
    do \
    { \
        union osdlr_el1 tmp = read_osdlr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_osdlr_el1(tmp); \
    } while (0)
