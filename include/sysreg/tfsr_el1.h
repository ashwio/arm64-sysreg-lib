...
static const union tfsr_el1 TFSR_EL1_SAFEVAL =
{0
};



static inline union tfsr_el1 read_tfsr_el1( void )
{
    union tfsr_el1 tmp;
    __asm(
        "MRS %0, s3_0_c5_c6_0"
...
static inline void unsafe_write_tfsr_el1( union tfsr_el1 val )
{
    __asm(
        "MSR s3_0_c5_c6_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tfsr_el1( ... ) \
    do \
    { \
        union tfsr_el1 tmp = TFSR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tfsr_el1(tmp); \
    } while (0)



#define read_modify_write_tfsr_el1( ... ) \
    do \
    { \
        union tfsr_el1 tmp = read_tfsr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tfsr_el1(tmp); \
    } while (0)
