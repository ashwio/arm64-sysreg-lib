...
static const union vtcr_el2 VTCR_EL2_SAFEVAL =
{
    .res1_31 = 1,
};
...
static inline union vtcr_el2 read_vtcr_el2( void )
{
    union vtcr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c2_c1_2"
...
static inline void unsafe_write_vtcr_el2( union vtcr_el2 val )
{
    __asm(
        "MSR s3_4_c2_c1_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_vtcr_el2( ... ) \
    do \
    { \
        union vtcr_el2 tmp = VTCR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vtcr_el2(tmp); \
    } while (0)



#define read_modify_write_vtcr_el2( ... ) \
    do \
    { \
        union vtcr_el2 tmp = read_vtcr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vtcr_el2(tmp); \
    } while (0)
