...
static const union tpidr_el0 TPIDR_EL0_SAFEVAL =
{0
};



static inline union tpidr_el0 read_tpidr_el0( void )
{
    union tpidr_el0 tmp;
    __asm(
        "MRS %0, s3_3_c13_c0_2"
...
static inline void unsafe_write_tpidr_el0( union tpidr_el0 val )
{
    __asm(
        "MSR s3_3_c13_c0_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tpidr_el0( ... ) \
    do \
    { \
        union tpidr_el0 tmp = TPIDR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tpidr_el0(tmp); \
    } while (0)



#define read_modify_write_tpidr_el0( ... ) \
    do \
    { \
        union tpidr_el0 tmp = read_tpidr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tpidr_el0(tmp); \
    } while (0)
