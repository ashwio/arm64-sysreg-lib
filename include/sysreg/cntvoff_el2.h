...
union cntvoff_el2
{
    ...
    __extension__ struct {
        u64 Virtual_offset : 64;
    };
};



static const union cntvoff_el2 CNTVOFF_EL2_SAFEVAL =
{0
};



static inline union cntvoff_el2 read_cntvoff_el2( void )
{
...
static inline void unsafe_write_cntvoff_el2( union cntvoff_el2 val )
{
    __asm(
        "MSR s3_4_c14_c0_3, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cntvoff_el2( ... ) \
    do \
    { \
        union cntvoff_el2 tmp = CNTVOFF_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntvoff_el2(tmp); \
    } while (0)



#define read_modify_write_cntvoff_el2( ... ) \
    do \
    { \
        union cntvoff_el2 tmp = read_cntvoff_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntvoff_el2(tmp); \
    } while (0)
