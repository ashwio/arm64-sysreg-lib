...
static const union ttbr0_el2 TTBR0_EL2_SAFEVAL =
{0
};



static inline union ttbr0_el2 read_ttbr0_el2( void )
{
    union ttbr0_el2 tmp;
    __asm(
        "MRS %0, s3_4_c2_c0_0"
...
static inline void unsafe_write_ttbr0_el2( union ttbr0_el2 val )
{
    __asm(
        "MSR s3_4_c2_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_ttbr0_el2( ... ) \
    do \
    { \
        union ttbr0_el2 tmp = TTBR0_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr0_el2(tmp); \
    } while (0)



#define read_modify_write_ttbr0_el2( ... ) \
    do \
    { \
        union ttbr0_el2 tmp = read_ttbr0_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr0_el2(tmp); \
    } while (0)
