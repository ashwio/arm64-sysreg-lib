...
static const union cnthp_cval_el2 CNTHP_CVAL_EL2_SAFEVAL =
{0
};



static inline union cnthp_cval_el2 read_cnthp_cval_el2( void )
{
...
static inline void unsafe_write_cnthp_cval_el2( union cnthp_cval_el2 val )
{
    __asm(
        "MSR s3_4_c14_c2_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cnthp_cval_el2( ... ) \
    do \
    { \
        union cnthp_cval_el2 tmp = CNTHP_CVAL_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cnthp_cval_el2(tmp); \
    } while (0)



#define read_modify_write_cnthp_cval_el2( ... ) \
    do \
    { \
        union cnthp_cval_el2 tmp = read_cnthp_cval_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cnthp_cval_el2(tmp); \
    } while (0)
