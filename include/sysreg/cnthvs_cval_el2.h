...
static const union cnthvs_cval_el2 CNTHVS_CVAL_EL2_SAFEVAL =
{0
};



static inline union cnthvs_cval_el2 read_cnthvs_cval_el2( void )
{
...
static inline void unsafe_write_cnthvs_cval_el2( union cnthvs_cval_el2 val )
{
    __asm(
        "MSR s3_4_c14_c4_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cnthvs_cval_el2( ... ) \
    do \
    { \
        union cnthvs_cval_el2 tmp = CNTHVS_CVAL_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cnthvs_cval_el2(tmp); \
    } while (0)



#define read_modify_write_cnthvs_cval_el2( ... ) \
    do \
    { \
        union cnthvs_cval_el2 tmp = read_cnthvs_cval_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cnthvs_cval_el2(tmp); \
    } while (0)
