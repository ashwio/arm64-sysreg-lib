...
static const union hacr_el2 HACR_EL2_SAFEVAL =
{0
};



static inline union hacr_el2 read_hacr_el2( void )
{
    union hacr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c1_c1_7"
...
static inline void unsafe_write_hacr_el2( union hacr_el2 val )
{
    __asm(
        "MSR s3_4_c1_c1_7, %0"
        : /**/ : "r" (val._)
...
#define safe_write_hacr_el2( ... ) \
    do \
    { \
        union hacr_el2 tmp = HACR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hacr_el2(tmp); \
    } while (0)



#define read_modify_write_hacr_el2( ... ) \
    do \
    { \
        union hacr_el2 tmp = read_hacr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hacr_el2(tmp); \
    } while (0)
