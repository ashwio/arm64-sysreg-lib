...
static const union oslar_el1 OSLAR_EL1_SAFEVAL =
{0
};
...
static inline void unsafe_write_oslar_el1( union oslar_el1 val )
{
    __asm(
        "MSR s2_0_c1_c0_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_oslar_el1( ... ) \
    do \
    { \
        union oslar_el1 tmp = OSLAR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_oslar_el1(tmp); \
    } while (0)
