...
static const union spsr_abt SPSR_ABT_SAFEVAL =
{0
};



static inline union spsr_abt read_spsr_abt( void )
{
    union spsr_abt tmp;
    __asm(
        "MRS %0, s3_4_c4_c3_1"
...
static inline void unsafe_write_spsr_abt( union spsr_abt val )
{
    __asm(
        "MSR s3_4_c4_c3_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_spsr_abt( ... ) \
    do \
    { \
        union spsr_abt tmp = SPSR_ABT_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_abt(tmp); \
    } while (0)



#define read_modify_write_spsr_abt( ... ) \
    do \
    { \
        union spsr_abt tmp = read_spsr_abt(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_abt(tmp); \
    } while (0)
