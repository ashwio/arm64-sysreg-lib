...
static inline union tcr_el3 read_tcr_el3( void )
{
    union tcr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c2_c0_2"
...
static inline void unsafe_write_tcr_el3( union tcr_el3 val )
{
    __asm(
        "MSR s3_6_c2_c0_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tcr_el3( ... ) \
    do \
    { \
        union tcr_el3 tmp = TCR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tcr_el3(tmp); \
    } while (0)



#define read_modify_write_tcr_el3( ... ) \
    do \
    { \
        union tcr_el3 tmp = read_tcr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tcr_el3(tmp); \
    } while (0)
