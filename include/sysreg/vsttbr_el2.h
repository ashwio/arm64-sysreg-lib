...
static const union vsttbr_el2 VSTTBR_EL2_SAFEVAL =
{0
};



static inline union vsttbr_el2 read_vsttbr_el2( void )
{
...
static inline void unsafe_write_vsttbr_el2( union vsttbr_el2 val )
{
    __asm(
        "MSR s3_4_c2_c6_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_vsttbr_el2( ... ) \
    do \
    { \
        union vsttbr_el2 tmp = VSTTBR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vsttbr_el2(tmp); \
    } while (0)



#define read_modify_write_vsttbr_el2( ... ) \
    do \
    { \
        union vsttbr_el2 tmp = read_vsttbr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vsttbr_el2(tmp); \
    } while (0)
