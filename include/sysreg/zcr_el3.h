...
static const union zcr_el3 ZCR_EL3_SAFEVAL =
{0
};



static inline union zcr_el3 read_zcr_el3( void )
{
    union zcr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c1_c2_0"
...
static inline void unsafe_write_zcr_el3( union zcr_el3 val )
{
    __asm(
        "MSR s3_6_c1_c2_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_zcr_el3( ... ) \
    do \
    { \
        union zcr_el3 tmp = ZCR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_zcr_el3(tmp); \
    } while (0)



#define read_modify_write_zcr_el3( ... ) \
    do \
    { \
        union zcr_el3 tmp = read_zcr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_zcr_el3(tmp); \
    } while (0)
