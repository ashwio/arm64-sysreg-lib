...
static const union dbgdtrtx_el0 DBGDTRTX_EL0_SAFEVAL =
{0
};
...
static inline void unsafe_write_dbgdtrtx_el0( union dbgdtrtx_el0 val )
{
    __asm(
        "MSR s2_3_c0_c5_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_dbgdtrtx_el0( ... ) \
    do \
    { \
        union dbgdtrtx_el0 tmp = DBGDTRTX_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dbgdtrtx_el0(tmp); \
    } while (0)
