...
static const union rmr_el1 RMR_EL1_SAFEVAL =
{
    .aa64 = 1,
};
...
static inline union rmr_el1 read_rmr_el1( void )
{
    union rmr_el1 tmp;
    __asm(
        "MRS %0, s3_0_c12_c0_2"
...
static inline void unsafe_write_rmr_el1( union rmr_el1 val )
{
    __asm(
        "MSR s3_0_c12_c0_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_rmr_el1( ... ) \
    do \
    { \
        union rmr_el1 tmp = RMR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_rmr_el1(tmp); \
    } while (0)



#define read_modify_write_rmr_el1( ... ) \
    do \
    { \
        union rmr_el1 tmp = read_rmr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_rmr_el1(tmp); \
    } while (0)
