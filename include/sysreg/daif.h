...
static const union daif DAIF_SAFEVAL =
{0
};



static inline union daif read_daif( void )
{
    union daif tmp;
    __asm(
        "MRS %0, s3_3_c4_c2_1"
...
static inline void unsafe_write_daif( union daif val )
{
    __asm(
        "MSR s3_3_c4_c2_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_daif( ... ) \
    do \
    { \
        union daif tmp = DAIF_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_daif(tmp); \
    } while (0)



#define read_modify_write_daif( ... ) \
    do \
    { \
        union daif tmp = read_daif(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_daif(tmp); \
    } while (0)
