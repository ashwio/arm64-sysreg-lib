...
static const union zcr_el1 ZCR_EL1_SAFEVAL =
{0
};



static inline union zcr_el1 read_zcr_el1( void )
{
    union zcr_el1 tmp;
    __asm(
        "MRS %0, s3_0_c1_c2_0"
...
static inline void unsafe_write_zcr_el1( union zcr_el1 val )
{
    __asm(
        "MSR s3_0_c1_c2_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_zcr_el1( ... ) \
    do \
    { \
        union zcr_el1 tmp = ZCR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_zcr_el1(tmp); \
    } while (0)



#define read_modify_write_zcr_el1( ... ) \
    do \
    { \
        union zcr_el1 tmp = read_zcr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_zcr_el1(tmp); \
    } while (0)
