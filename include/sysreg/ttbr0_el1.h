...
static const union ttbr0_el1 TTBR0_EL1_SAFEVAL =
{0
};



static inline union ttbr0_el1 read_ttbr0_el1( void )
{
    union ttbr0_el1 tmp;
    __asm(
        "MRS %0, s3_0_c2_c0_0"
...
static inline void unsafe_write_ttbr0_el1( union ttbr0_el1 val )
{
    __asm(
        "MSR s3_0_c2_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_ttbr0_el1( ... ) \
    do \
    { \
        union ttbr0_el1 tmp = TTBR0_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr0_el1(tmp); \
    } while (0)



#define read_modify_write_ttbr0_el1( ... ) \
    do \
    { \
        union ttbr0_el1 tmp = read_ttbr0_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr0_el1(tmp); \
    } while (0)
