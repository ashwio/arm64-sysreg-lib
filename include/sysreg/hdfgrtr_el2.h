...
static const union hdfgrtr_el2 HDFGRTR_EL2_SAFEVAL =
{0
};



static inline union hdfgrtr_el2 read_hdfgrtr_el2( void )
{
...
static inline void unsafe_write_hdfgrtr_el2( union hdfgrtr_el2 val )
{
    __asm(
        "MSR s3_4_c3_c1_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_hdfgrtr_el2( ... ) \
    do \
    { \
        union hdfgrtr_el2 tmp = HDFGRTR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hdfgrtr_el2(tmp); \
    } while (0)



#define read_modify_write_hdfgrtr_el2( ... ) \
    do \
    { \
        union hdfgrtr_el2 tmp = read_hdfgrtr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hdfgrtr_el2(tmp); \
    } while (0)
