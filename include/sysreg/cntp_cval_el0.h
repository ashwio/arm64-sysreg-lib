...
static const union cntp_cval_el0 CNTP_CVAL_EL0_SAFEVAL =
{0
};



static inline union cntp_cval_el0 read_cntp_cval_el0( void )
{
...
static inline void unsafe_write_cntp_cval_el0( union cntp_cval_el0 val )
{
    __asm(
        "MSR s3_3_c14_c2_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cntp_cval_el0( ... ) \
    do \
    { \
        union cntp_cval_el0 tmp = CNTP_CVAL_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntp_cval_el0(tmp); \
    } while (0)



#define read_modify_write_cntp_cval_el0( ... ) \
    do \
    { \
        union cntp_cval_el0 tmp = read_cntp_cval_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntp_cval_el0(tmp); \
    } while (0)
