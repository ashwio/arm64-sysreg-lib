...
static const union cntkctl_el1 CNTKCTL_EL1_SAFEVAL =
{0
};



static inline union cntkctl_el1 read_cntkctl_el1( void )
{
...
static inline void unsafe_write_cntkctl_el1( union cntkctl_el1 val )
{
    __asm(
        "MSR s3_0_c14_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cntkctl_el1( ... ) \
    do \
    { \
        union cntkctl_el1 tmp = CNTKCTL_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntkctl_el1(tmp); \
    } while (0)



#define read_modify_write_cntkctl_el1( ... ) \
    do \
    { \
        union cntkctl_el1 tmp = read_cntkctl_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntkctl_el1(tmp); \
    } while (0)
