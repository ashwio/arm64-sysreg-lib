...
static const union afsr0_el2 AFSR0_EL2_SAFEVAL =
{0
};



static inline union afsr0_el2 read_afsr0_el2( void )
{
    union afsr0_el2 tmp;
    __asm(
        "MRS %0, s3_4_c5_c1_0"
...
static inline void unsafe_write_afsr0_el2( union afsr0_el2 val )
{
    __asm(
        "MSR s3_4_c5_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_afsr0_el2( ... ) \
    do \
    { \
        union afsr0_el2 tmp = AFSR0_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_afsr0_el2(tmp); \
    } while (0)



#define read_modify_write_afsr0_el2( ... ) \
    do \
    { \
        union afsr0_el2 tmp = read_afsr0_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_afsr0_el2(tmp); \
    } while (0)
