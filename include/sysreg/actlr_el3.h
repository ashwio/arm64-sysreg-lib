...
static const union actlr_el3 ACTLR_EL3_SAFEVAL =
{0
};



static inline union actlr_el3 read_actlr_el3( void )
{
    union actlr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c1_c0_1"
...
static inline void unsafe_write_actlr_el3( union actlr_el3 val )
{
    __asm(
        "MSR s3_6_c1_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_actlr_el3( ... ) \
    do \
    { \
        union actlr_el3 tmp = ACTLR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_actlr_el3(tmp); \
    } while (0)



#define read_modify_write_actlr_el3( ... ) \
    do \
    { \
        union actlr_el3 tmp = read_actlr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_actlr_el3(tmp); \
    } while (0)
