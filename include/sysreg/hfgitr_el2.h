...
static const union hfgitr_el2 HFGITR_EL2_SAFEVAL =
{0
};



static inline union hfgitr_el2 read_hfgitr_el2( void )
{
...
static inline void unsafe_write_hfgitr_el2( union hfgitr_el2 val )
{
    __asm(
        "MSR s3_4_c1_c1_6, %0"
        : /**/ : "r" (val._)
...
#define safe_write_hfgitr_el2( ... ) \
    do \
    { \
        union hfgitr_el2 tmp = HFGITR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hfgitr_el2(tmp); \
    } while (0)



#define read_modify_write_hfgitr_el2( ... ) \
    do \
    { \
        union hfgitr_el2 tmp = read_hfgitr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hfgitr_el2(tmp); \
    } while (0)
