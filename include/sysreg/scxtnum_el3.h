...
static const union scxtnum_el3 SCXTNUM_EL3_SAFEVAL =
{0
};



static inline union scxtnum_el3 read_scxtnum_el3( void )
{
...
static inline void unsafe_write_scxtnum_el3( union scxtnum_el3 val )
{
    __asm(
        "MSR s3_6_c13_c0_7, %0"
        : /**/ : "r" (val._)
...
#define safe_write_scxtnum_el3( ... ) \
    do \
    { \
        union scxtnum_el3 tmp = SCXTNUM_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_scxtnum_el3(tmp); \
    } while (0)



#define read_modify_write_scxtnum_el3( ... ) \
    do \
    { \
        union scxtnum_el3 tmp = read_scxtnum_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_scxtnum_el3(tmp); \
    } while (0)
