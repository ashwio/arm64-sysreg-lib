...
union far_el3
{
    ...
    __extension__ struct {
        u64 Faulting_Virtual_Address_for_synchronous_exceptions_taken_to_EL3 : 64;
    };
};



static const union far_el3 FAR_EL3_SAFEVAL =
{0
};



static inline union far_el3 read_far_el3( void )
{
    union far_el3 tmp;
    __asm(
        "MRS %0, s3_6_c6_c0_0"
...
static inline void unsafe_write_far_el3( union far_el3 val )
{
    __asm(
        "MSR s3_6_c6_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_far_el3( ... ) \
    do \
    { \
        union far_el3 tmp = FAR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_far_el3(tmp); \
    } while (0)



#define read_modify_write_far_el3( ... ) \
    do \
    { \
        union far_el3 tmp = read_far_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_far_el3(tmp); \
    } while (0)
