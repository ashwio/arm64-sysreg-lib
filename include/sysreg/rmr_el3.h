...
static const union rmr_el3 RMR_EL3_SAFEVAL =
{
    .aa64 = 1,
};
...
static inline union rmr_el3 read_rmr_el3( void )
{
    union rmr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c12_c0_2"
...
static inline void unsafe_write_rmr_el3( union rmr_el3 val )
{
    __asm(
        "MSR s3_6_c12_c0_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_rmr_el3( ... ) \
    do \
    { \
        union rmr_el3 tmp = RMR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_rmr_el3(tmp); \
    } while (0)



#define read_modify_write_rmr_el3( ... ) \
    do \
    { \
        union rmr_el3 tmp = read_rmr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_rmr_el3(tmp); \
    } while (0)
