...
static const union fpexc32_el2 FPEXC32_EL2_SAFEVAL =
{0
};



static inline union fpexc32_el2 read_fpexc32_el2( void )
{
...
static inline void unsafe_write_fpexc32_el2( union fpexc32_el2 val )
{
    __asm(
        "MSR s3_4_c5_c3_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_fpexc32_el2( ... ) \
    do \
    { \
        union fpexc32_el2 tmp = FPEXC32_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_fpexc32_el2(tmp); \
    } while (0)



#define read_modify_write_fpexc32_el2( ... ) \
    do \
    { \
        union fpexc32_el2 tmp = read_fpexc32_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_fpexc32_el2(tmp); \
    } while (0)
