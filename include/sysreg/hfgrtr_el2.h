...
static const union hfgrtr_el2 HFGRTR_EL2_SAFEVAL =
{0
};



static inline union hfgrtr_el2 read_hfgrtr_el2( void )
{
...
static inline void unsafe_write_hfgrtr_el2( union hfgrtr_el2 val )
{
    __asm(
        "MSR s3_4_c1_c1_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_hfgrtr_el2( ... ) \
    do \
    { \
        union hfgrtr_el2 tmp = HFGRTR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hfgrtr_el2(tmp); \
    } while (0)



#define read_modify_write_hfgrtr_el2( ... ) \
    do \
    { \
        union hfgrtr_el2 tmp = read_hfgrtr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hfgrtr_el2(tmp); \
    } while (0)
