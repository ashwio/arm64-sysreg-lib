...
static const union csselr_el1 CSSELR_EL1_SAFEVAL =
{0
};



static inline union csselr_el1 read_csselr_el1( void )
{
...
static inline void unsafe_write_csselr_el1( union csselr_el1 val )
{
    __asm(
        "MSR s3_2_c0_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_csselr_el1( ... ) \
    do \
    { \
        union csselr_el1 tmp = CSSELR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_csselr_el1(tmp); \
    } while (0)



#define read_modify_write_csselr_el1( ... ) \
    do \
    { \
        union csselr_el1 tmp = read_csselr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_csselr_el1(tmp); \
    } while (0)
