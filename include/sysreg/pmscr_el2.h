...
static const union pmscr_el2 PMSCR_EL2_SAFEVAL =
{0
};



static inline union pmscr_el2 read_pmscr_el2( void )
{
    union pmscr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c9_c9_0"
...
static inline void unsafe_write_pmscr_el2( union pmscr_el2 val )
{
    __asm(
        "MSR s3_4_c9_c9_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pmscr_el2( ... ) \
    do \
    { \
        union pmscr_el2 tmp = PMSCR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmscr_el2(tmp); \
    } while (0)



#define read_modify_write_pmscr_el2( ... ) \
    do \
    { \
        union pmscr_el2 tmp = read_pmscr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmscr_el2(tmp); \
    } while (0)
