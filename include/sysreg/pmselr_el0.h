...
static const union pmselr_el0 PMSELR_EL0_SAFEVAL =
{0
};



static inline union pmselr_el0 read_pmselr_el0( void )
{
...
static inline void unsafe_write_pmselr_el0( union pmselr_el0 val )
{
    __asm(
        "MSR s3_3_c9_c12_5, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pmselr_el0( ... ) \
    do \
    { \
        union pmselr_el0 tmp = PMSELR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmselr_el0(tmp); \
    } while (0)



#define read_modify_write_pmselr_el0( ... ) \
    do \
    { \
        union pmselr_el0 tmp = read_pmselr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmselr_el0(tmp); \
    } while (0)
