...
static const union contextidr_el1 CONTEXTIDR_EL1_SAFEVAL =
{0
};



static inline union contextidr_el1 read_contextidr_el1( void )
{
...
static inline void unsafe_write_contextidr_el1( union contextidr_el1 val )
{
    __asm(
        "MSR s3_0_c13_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_contextidr_el1( ... ) \
    do \
    { \
        union contextidr_el1 tmp = CONTEXTIDR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_contextidr_el1(tmp); \
    } while (0)



#define read_modify_write_contextidr_el1( ... ) \
    do \
    { \
        union contextidr_el1 tmp = read_contextidr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_contextidr_el1(tmp); \
    } while (0)
