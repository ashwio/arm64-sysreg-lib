...
static inline union sctlr_el1 read_sctlr_el1( void )
{
    union sctlr_el1 tmp;
    __asm(
        "MRS %0, s3_0_c1_c0_0"
...
static inline void unsafe_write_sctlr_el1( union sctlr_el1 val )
{
    __asm(
        "MSR s3_0_c1_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_sctlr_el1( ... ) \
    do \
    { \
        union sctlr_el1 tmp = SCTLR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sctlr_el1(tmp); \
    } while (0)



#define read_modify_write_sctlr_el1( ... ) \
    do \
    { \
        union sctlr_el1 tmp = read_sctlr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sctlr_el1(tmp); \
    } while (0)
