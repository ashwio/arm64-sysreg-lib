...
static const union pmuserenr_el0 PMUSERENR_EL0_SAFEVAL =
{0
};



static inline union pmuserenr_el0 read_pmuserenr_el0( void )
{
...
static inline void unsafe_write_pmuserenr_el0( union pmuserenr_el0 val )
{
    __asm(
        "MSR s3_3_c9_c14_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pmuserenr_el0( ... ) \
    do \
    { \
        union pmuserenr_el0 tmp = PMUSERENR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmuserenr_el0(tmp); \
    } while (0)



#define read_modify_write_pmuserenr_el0( ... ) \
    do \
    { \
        union pmuserenr_el0 tmp = read_pmuserenr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmuserenr_el0(tmp); \
    } while (0)
