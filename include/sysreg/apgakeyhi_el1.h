...
static const union apgakeyhi_el1 APGAKEYHI_EL1_SAFEVAL =
{0
};



static inline union apgakeyhi_el1 read_apgakeyhi_el1( void )
{
...
static inline void unsafe_write_apgakeyhi_el1( union apgakeyhi_el1 val )
{
    __asm(
        "MSR s3_0_c2_c3_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_apgakeyhi_el1( ... ) \
    do \
    { \
        union apgakeyhi_el1 tmp = APGAKEYHI_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_apgakeyhi_el1(tmp); \
    } while (0)



#define read_modify_write_apgakeyhi_el1( ... ) \
    do \
    { \
        union apgakeyhi_el1 tmp = read_apgakeyhi_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_apgakeyhi_el1(tmp); \
    } while (0)
