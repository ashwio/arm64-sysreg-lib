...
static const union pmsicr_el1 PMSICR_EL1_SAFEVAL =
{0
};



static inline union pmsicr_el1 read_pmsicr_el1( void )
{
...
static inline void unsafe_write_pmsicr_el1( union pmsicr_el1 val )
{
    __asm(
        "MSR s3_0_c9_c9_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pmsicr_el1( ... ) \
    do \
    { \
        union pmsicr_el1 tmp = PMSICR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmsicr_el1(tmp); \
    } while (0)



#define read_modify_write_pmsicr_el1( ... ) \
    do \
    { \
        union pmsicr_el1 tmp = read_pmsicr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmsicr_el1(tmp); \
    } while (0)
