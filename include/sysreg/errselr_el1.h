...
static const union errselr_el1 ERRSELR_EL1_SAFEVAL =
{0
};



static inline union errselr_el1 read_errselr_el1( void )
{
...
static inline void unsafe_write_errselr_el1( union errselr_el1 val )
{
    __asm(
        "MSR s3_0_c5_c3_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_errselr_el1( ... ) \
    do \
    { \
        union errselr_el1 tmp = ERRSELR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_errselr_el1(tmp); \
    } while (0)



#define read_modify_write_errselr_el1( ... ) \
    do \
    { \
        union errselr_el1 tmp = read_errselr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_errselr_el1(tmp); \
    } while (0)
