...
static const union ssbs SSBS_SAFEVAL =
{0
};



static inline union ssbs read_ssbs( void )
{
    union ssbs tmp;
    __asm(
        "MRS %0, s3_3_c4_c2_6"
...
static inline void unsafe_write_ssbs( union ssbs val )
{
    __asm(
        "MSR s3_3_c4_c2_6, %0"
        : /**/ : "r" (val._)
...
#define safe_write_ssbs( ... ) \
    do \
    { \
        union ssbs tmp = SSBS_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ssbs(tmp); \
    } while (0)



#define read_modify_write_ssbs( ... ) \
    do \
    { \
        union ssbs tmp = read_ssbs(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ssbs(tmp); \
    } while (0)
