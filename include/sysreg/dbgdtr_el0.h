...
static const union dbgdtr_el0 DBGDTR_EL0_SAFEVAL =
{0
};



static inline union dbgdtr_el0 read_dbgdtr_el0( void )
{
...
static inline void unsafe_write_dbgdtr_el0( union dbgdtr_el0 val )
{
    __asm(
        "MSR s2_3_c0_c4_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_dbgdtr_el0( ... ) \
    do \
    { \
        union dbgdtr_el0 tmp = DBGDTR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dbgdtr_el0(tmp); \
    } while (0)



#define read_modify_write_dbgdtr_el0( ... ) \
    do \
    { \
        union dbgdtr_el0 tmp = read_dbgdtr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dbgdtr_el0(tmp); \
    } while (0)
