...
static const union icc_ctlr_el3 ICC_CTLR_EL3_SAFEVAL =
{0
};



static inline union icc_ctlr_el3 read_icc_ctlr_el3( void )
{
...
static inline void unsafe_write_icc_ctlr_el3( union icc_ctlr_el3 val )
{
    __asm(
...
#define safe_write_icc_ctlr_el3( ... ) \
    do \
    { \
        union icc_ctlr_el3 tmp = ICC_CTLR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_icc_ctlr_el3(tmp); \
    } while (0)



#define read_modify_write_icc_ctlr_el3( ... ) \
    do \
    { \
        union icc_ctlr_el3 tmp = read_icc_ctlr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_icc_ctlr_el3(tmp); \
    } while (0)
