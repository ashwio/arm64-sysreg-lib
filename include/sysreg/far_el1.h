...
union far_el1
{
    ...
    __extension__ struct {
        u64 Faulting_Virtual_Address_for_synchronous_exceptions_taken_to_EL1 : 64;
    };
};



static const union far_el1 FAR_EL1_SAFEVAL =
{0
};



static inline union far_el1 read_far_el1( void )
{
    union far_el1 tmp;
    __asm(
        "MRS %0, s3_0_c6_c0_0"
...
static inline void unsafe_write_far_el1( union far_el1 val )
{
    __asm(
        "MSR s3_0_c6_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_far_el1( ... ) \
    do \
    { \
        union far_el1 tmp = FAR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_far_el1(tmp); \
    } while (0)



#define read_modify_write_far_el1( ... ) \
    do \
    { \
        union far_el1 tmp = read_far_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_far_el1(tmp); \
    } while (0)
