...
static const union spsr_und SPSR_UND_SAFEVAL =
{0
};



static inline union spsr_und read_spsr_und( void )
{
    union spsr_und tmp;
    __asm(
        "MRS %0, s3_4_c4_c3_2"
...
static inline void unsafe_write_spsr_und( union spsr_und val )
{
    __asm(
        "MSR s3_4_c4_c3_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_spsr_und( ... ) \
    do \
    { \
        union spsr_und tmp = SPSR_UND_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_und(tmp); \
    } while (0)



#define read_modify_write_spsr_und( ... ) \
    do \
    { \
        union spsr_und tmp = read_spsr_und(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_und(tmp); \
    } while (0)
