...
static const union uao UAO_SAFEVAL =
{0
};



static inline union uao read_uao( void )
{
    union uao tmp;
    __asm(
        "MRS %0, s3_0_c4_c2_4"
...
static inline void unsafe_write_uao( union uao val )
{
    __asm(
        "MSR s3_0_c4_c2_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_uao( ... ) \
    do \
    { \
        union uao tmp = UAO_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_uao(tmp); \
    } while (0)



#define read_modify_write_uao( ... ) \
    do \
    { \
        union uao tmp = read_uao(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_uao(tmp); \
    } while (0)
