...
static const union oseccr_el1 OSECCR_EL1_SAFEVAL =
{0
};



static inline union oseccr_el1 read_oseccr_el1( void )
{
...
static inline void unsafe_write_oseccr_el1( union oseccr_el1 val )
{
    __asm(
        "MSR s2_0_c0_c6_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_oseccr_el1( ... ) \
    do \
    { \
        union oseccr_el1 tmp = OSECCR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_oseccr_el1(tmp); \
    } while (0)



#define read_modify_write_oseccr_el1( ... ) \
    do \
    { \
        union oseccr_el1 tmp = read_oseccr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_oseccr_el1(tmp); \
    } while (0)
