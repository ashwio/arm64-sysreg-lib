...
static const union scr_el3 SCR_EL3_SAFEVAL =
{
    .res1_5_4 = 3,
    .rw = 1,
};
...
static inline union scr_el3 read_scr_el3( void )
{
    union scr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c1_c1_0"
...
static inline void unsafe_write_scr_el3( union scr_el3 val )
{
    __asm(
        "MSR s3_6_c1_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_scr_el3( ... ) \
    do \
    { \
        union scr_el3 tmp = SCR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_scr_el3(tmp); \
    } while (0)



#define read_modify_write_scr_el3( ... ) \
    do \
    { \
        union scr_el3 tmp = read_scr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_scr_el3(tmp); \
    } while (0)
