...
static const union tpidr_el2 TPIDR_EL2_SAFEVAL =
{0
};



static inline union tpidr_el2 read_tpidr_el2( void )
{
    union tpidr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c13_c0_2"
...
static inline void unsafe_write_tpidr_el2( union tpidr_el2 val )
{
    __asm(
        "MSR s3_4_c13_c0_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tpidr_el2( ... ) \
    do \
    { \
        union tpidr_el2 tmp = TPIDR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tpidr_el2(tmp); \
    } while (0)



#define read_modify_write_tpidr_el2( ... ) \
    do \
    { \
        union tpidr_el2 tmp = read_tpidr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tpidr_el2(tmp); \
    } while (0)
