...
static const union sp_el1 SP_EL1_SAFEVAL =
{0
};



static inline union sp_el1 read_sp_el1( void )
{
    union sp_el1 tmp;
    __asm(
        "MRS %0, s3_4_c4_c1_0"
...
static inline void unsafe_write_sp_el1( union sp_el1 val )
{
    __asm(
        "MSR s3_4_c4_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_sp_el1( ... ) \
    do \
    { \
        union sp_el1 tmp = SP_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sp_el1(tmp); \
    } while (0)



#define read_modify_write_sp_el1( ... ) \
    do \
    { \
        union sp_el1 tmp = read_sp_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sp_el1(tmp); \
    } while (0)
