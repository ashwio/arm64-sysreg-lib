...
static const union icc_eoir1_el1 ICC_EOIR1_EL1_SAFEVAL =
{0
};
...
static inline void unsafe_write_icc_eoir1_el1( union icc_eoir1_el1 val )
{
    __asm(
...
#define safe_write_icc_eoir1_el1( ... ) \
    do \
    { \
        union icc_eoir1_el1 tmp = ICC_EOIR1_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_icc_eoir1_el1(tmp); \
    } while (0)
