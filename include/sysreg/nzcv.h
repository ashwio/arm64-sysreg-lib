...
static const union nzcv NZCV_SAFEVAL =
{0
};



static inline union nzcv read_nzcv( void )
{
    union nzcv tmp;
    __asm(
        "MRS %0, s3_3_c4_c2_0"
...
static inline void unsafe_write_nzcv( union nzcv val )
{
    __asm(
        "MSR s3_3_c4_c2_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_nzcv( ... ) \
    do \
    { \
        union nzcv tmp = NZCV_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_nzcv(tmp); \
    } while (0)



#define read_modify_write_nzcv( ... ) \
    do \
    { \
        union nzcv tmp = read_nzcv(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_nzcv(tmp); \
    } while (0)
