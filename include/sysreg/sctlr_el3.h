...
static inline union sctlr_el3 read_sctlr_el3( void )
{
    union sctlr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c1_c0_0"
...
static inline void unsafe_write_sctlr_el3( union sctlr_el3 val )
{
    __asm(
        "MSR s3_6_c1_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_sctlr_el3( ... ) \
    do \
    { \
        union sctlr_el3 tmp = SCTLR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sctlr_el3(tmp); \
    } while (0)



#define read_modify_write_sctlr_el3( ... ) \
    do \
    { \
        union sctlr_el3 tmp = read_sctlr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sctlr_el3(tmp); \
    } while (0)
