...
static const union rmr_el2 RMR_EL2_SAFEVAL =
{
    .aa64 = 1,
};
...
static inline union rmr_el2 read_rmr_el2( void )
{
    union rmr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c12_c0_2"
...
static inline void unsafe_write_rmr_el2( union rmr_el2 val )
{
    __asm(
        "MSR s3_4_c12_c0_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_rmr_el2( ... ) \
    do \
    { \
        union rmr_el2 tmp = RMR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_rmr_el2(tmp); \
    } while (0)



#define read_modify_write_rmr_el2( ... ) \
    do \
    { \
        union rmr_el2 tmp = read_rmr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_rmr_el2(tmp); \
    } while (0)
