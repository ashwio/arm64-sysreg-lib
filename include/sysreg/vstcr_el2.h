...
static const union vstcr_el2 VSTCR_EL2_SAFEVAL =
{
    .res1_31 = 1,
};
...
static inline union vstcr_el2 read_vstcr_el2( void )
{
    union vstcr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c2_c6_2"
...
static inline void unsafe_write_vstcr_el2( union vstcr_el2 val )
{
    __asm(
        "MSR s3_4_c2_c6_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_vstcr_el2( ... ) \
    do \
    { \
        union vstcr_el2 tmp = VSTCR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vstcr_el2(tmp); \
    } while (0)



#define read_modify_write_vstcr_el2( ... ) \
    do \
    { \
        union vstcr_el2 tmp = read_vstcr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vstcr_el2(tmp); \
    } while (0)
