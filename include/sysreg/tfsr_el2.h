...
static const union tfsr_el2 TFSR_EL2_SAFEVAL =
{0
};



static inline union tfsr_el2 read_tfsr_el2( void )
{
    union tfsr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c5_c6_0"
...
static inline void unsafe_write_tfsr_el2( union tfsr_el2 val )
{
    __asm(
        "MSR s3_4_c5_c6_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tfsr_el2( ... ) \
    do \
    { \
        union tfsr_el2 tmp = TFSR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tfsr_el2(tmp); \
    } while (0)



#define read_modify_write_tfsr_el2( ... ) \
    do \
    { \
        union tfsr_el2 tmp = read_tfsr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tfsr_el2(tmp); \
    } while (0)
