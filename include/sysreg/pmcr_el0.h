...
static const union pmcr_el0 PMCR_EL0_SAFEVAL =
{
    .lc = 1,
};
...
static inline union pmcr_el0 read_pmcr_el0( void )
{
    union pmcr_el0 tmp;
    __asm(
        "MRS %0, s3_3_c9_c12_0"
...
static inline void unsafe_write_pmcr_el0( union pmcr_el0 val )
{
    __asm(
        "MSR s3_3_c9_c12_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pmcr_el0( ... ) \
    do \
    { \
        union pmcr_el0 tmp = PMCR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmcr_el0(tmp); \
    } while (0)



#define read_modify_write_pmcr_el0( ... ) \
    do \
    { \
        union pmcr_el0 tmp = read_pmcr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmcr_el0(tmp); \
    } while (0)
