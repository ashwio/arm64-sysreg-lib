...
static const union spsr_fiq SPSR_FIQ_SAFEVAL =
{0
};



static inline union spsr_fiq read_spsr_fiq( void )
{
    union spsr_fiq tmp;
    __asm(
        "MRS %0, s3_4_c4_c3_3"
...
static inline void unsafe_write_spsr_fiq( union spsr_fiq val )
{
    __asm(
        "MSR s3_4_c4_c3_3, %0"
        : /**/ : "r" (val._)
...
#define safe_write_spsr_fiq( ... ) \
    do \
    { \
        union spsr_fiq tmp = SPSR_FIQ_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_fiq(tmp); \
    } while (0)



#define read_modify_write_spsr_fiq( ... ) \
    do \
    { \
        union spsr_fiq tmp = read_spsr_fiq(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_fiq(tmp); \
    } while (0)
