...
static const union lorn_el1 LORN_EL1_SAFEVAL =
{0
};



static inline union lorn_el1 read_lorn_el1( void )
{
    union lorn_el1 tmp;
    __asm(
        "MRS %0, s3_0_c10_c4_2"
...
static inline void unsafe_write_lorn_el1( union lorn_el1 val )
{
    __asm(
        "MSR s3_0_c10_c4_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_lorn_el1( ... ) \
    do \
    { \
        union lorn_el1 tmp = LORN_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_lorn_el1(tmp); \
    } while (0)



#define read_modify_write_lorn_el1( ... ) \
    do \
    { \
        union lorn_el1 tmp = read_lorn_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_lorn_el1(tmp); \
    } while (0)
