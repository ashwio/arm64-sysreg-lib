...
static const union vmpidr_el2 VMPIDR_EL2_SAFEVAL =
{
    .res1_31 = 1,
};
...
static inline union vmpidr_el2 read_vmpidr_el2( void )
{
...
static inline void unsafe_write_vmpidr_el2( union vmpidr_el2 val )
{
    __asm(
        "MSR s3_4_c0_c0_5, %0"
        : /**/ : "r" (val._)
...
#define safe_write_vmpidr_el2( ... ) \
    do \
    { \
        union vmpidr_el2 tmp = VMPIDR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vmpidr_el2(tmp); \
    } while (0)



#define read_modify_write_vmpidr_el2( ... ) \
    do \
    { \
        union vmpidr_el2 tmp = read_vmpidr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vmpidr_el2(tmp); \
    } while (0)
