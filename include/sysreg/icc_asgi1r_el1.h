...
static const union icc_asgi1r_el1 ICC_ASGI1R_EL1_SAFEVAL =
{0
};
...
static inline void unsafe_write_icc_asgi1r_el1( union icc_asgi1r_el1 val )
{
    __asm(
        "MSR s3_0_c12_c11_6, %0"
        : /**/ : "r" (val._)
    );
}
...
#define safe_write_icc_asgi1r_el1( ... ) \
    do \
    { \
        union icc_asgi1r_el1 tmp = ICC_ASGI1R_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_icc_asgi1r_el1(tmp); \
    } while (0)
