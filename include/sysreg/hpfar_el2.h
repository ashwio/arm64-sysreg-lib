...
static const union hpfar_el2 HPFAR_EL2_SAFEVAL =
{0
};



static inline union hpfar_el2 read_hpfar_el2( void )
{
    union hpfar_el2 tmp;
    __asm(
        "MRS %0, s3_4_c6_c0_4"
...
static inline void unsafe_write_hpfar_el2( union hpfar_el2 val )
{
    __asm(
        "MSR s3_4_c6_c0_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_hpfar_el2( ... ) \
    do \
    { \
        union hpfar_el2 tmp = HPFAR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hpfar_el2(tmp); \
    } while (0)



#define read_modify_write_hpfar_el2( ... ) \
    do \
    { \
        union hpfar_el2 tmp = read_hpfar_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hpfar_el2(tmp); \
    } while (0)
