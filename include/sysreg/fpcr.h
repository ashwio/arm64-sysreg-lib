...
static const union fpcr FPCR_SAFEVAL =
{0
};



static inline union fpcr read_fpcr( void )
{
    union fpcr tmp;
    __asm(
        "MRS %0, s3_3_c4_c4_0"
...
static inline void unsafe_write_fpcr( union fpcr val )
{
    __asm(
        "MSR s3_3_c4_c4_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_fpcr( ... ) \
    do \
    { \
        union fpcr tmp = FPCR_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_fpcr(tmp); \
    } while (0)



#define read_modify_write_fpcr( ... ) \
    do \
    { \
        union fpcr tmp = read_fpcr(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_fpcr(tmp); \
    } while (0)
