...
static const union fpsr FPSR_SAFEVAL =
{0
};



static inline union fpsr read_fpsr( void )
{
    union fpsr tmp;
    __asm(
        "MRS %0, s3_3_c4_c4_1"
...
static inline void unsafe_write_fpsr( union fpsr val )
{
    __asm(
        "MSR s3_3_c4_c4_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_fpsr( ... ) \
    do \
    { \
        union fpsr tmp = FPSR_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_fpsr(tmp); \
    } while (0)



#define read_modify_write_fpsr( ... ) \
    do \
    { \
        union fpsr tmp = read_fpsr(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_fpsr(tmp); \
    } while (0)
