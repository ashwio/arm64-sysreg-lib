...
static const union contextidr_el2 CONTEXTIDR_EL2_SAFEVAL =
{0
};



static inline union contextidr_el2 read_contextidr_el2( void )
{
...
static inline void unsafe_write_contextidr_el2( union contextidr_el2 val )
{
    __asm(
        "MSR s3_4_c13_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_contextidr_el2( ... ) \
    do \
    { \
        union contextidr_el2 tmp = CONTEXTIDR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_contextidr_el2(tmp); \
    } while (0)



#define read_modify_write_contextidr_el2( ... ) \
    do \
    { \
        union contextidr_el2 tmp = read_contextidr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_contextidr_el2(tmp); \
    } while (0)
