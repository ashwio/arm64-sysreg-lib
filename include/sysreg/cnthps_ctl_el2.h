...
static const union cnthps_ctl_el2 CNTHPS_CTL_EL2_SAFEVAL =
{0
};



static inline union cnthps_ctl_el2 read_cnthps_ctl_el2( void )
{
...
static inline void unsafe_write_cnthps_ctl_el2( union cnthps_ctl_el2 val )
{
    __asm(
        "MSR s3_4_c14_c5_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cnthps_ctl_el2( ... ) \
    do \
    { \
        union cnthps_ctl_el2 tmp = CNTHPS_CTL_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cnthps_ctl_el2(tmp); \
    } while (0)



#define read_modify_write_cnthps_ctl_el2( ... ) \
    do \
    { \
        union cnthps_ctl_el2 tmp = read_cnthps_ctl_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cnthps_ctl_el2(tmp); \
    } while (0)
