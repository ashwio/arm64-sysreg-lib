...
static const union hcr_el2 HCR_EL2_SAFEVAL =
{
    .rw = 1,
};
...
static inline union hcr_el2 read_hcr_el2( void )
{
    union hcr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c1_c1_0"
...
static inline void unsafe_write_hcr_el2( union hcr_el2 val )
{
    __asm(
        "MSR s3_4_c1_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_hcr_el2( ... ) \
    do \
    { \
        union hcr_el2 tmp = HCR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hcr_el2(tmp); \
    } while (0)



#define read_modify_write_hcr_el2( ... ) \
    do \
    { \
        union hcr_el2 tmp = read_hcr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_hcr_el2(tmp); \
    } while (0)
