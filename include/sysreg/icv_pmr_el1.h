...
static const union icv_pmr_el1 ICV_PMR_EL1_SAFEVAL =
{0
};



static inline union icv_pmr_el1 read_icv_pmr_el1( void )
{
...
static inline void unsafe_write_icv_pmr_el1( union icv_pmr_el1 val )
{
    __asm(
        "MSR s3_0_c4_c6_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_icv_pmr_el1( ... ) \
    do \
    { \
        union icv_pmr_el1 tmp = ICV_PMR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_icv_pmr_el1(tmp); \
    } while (0)



#define read_modify_write_icv_pmr_el1( ... ) \
    do \
    { \
        union icv_pmr_el1 tmp = read_icv_pmr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_icv_pmr_el1(tmp); \
    } while (0)
