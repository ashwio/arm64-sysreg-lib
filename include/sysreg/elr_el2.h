...
static const union elr_el2 ELR_EL2_SAFEVAL =
{0
};



static inline union elr_el2 read_elr_el2( void )
{
    union elr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c4_c0_1"
...
static inline void unsafe_write_elr_el2( union elr_el2 val )
{
    __asm(
        "MSR s3_4_c4_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_elr_el2( ... ) \
    do \
    { \
        union elr_el2 tmp = ELR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_elr_el2(tmp); \
    } while (0)



#define read_modify_write_elr_el2( ... ) \
    do \
    { \
        union elr_el2 tmp = read_elr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_elr_el2(tmp); \
    } while (0)
