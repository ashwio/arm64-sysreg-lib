...
static const union tco TCO_SAFEVAL =
{0
};



static inline union tco read_tco( void )
{
    union tco tmp;
    __asm(
        "MRS %0, s3_3_c4_c2_7"
...
static inline void unsafe_write_tco( union tco val )
{
    __asm(
        "MSR s3_3_c4_c2_7, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tco( ... ) \
    do \
    { \
        union tco tmp = TCO_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tco(tmp); \
    } while (0)



#define read_modify_write_tco( ... ) \
    do \
    { \
        union tco tmp = read_tco(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tco(tmp); \
    } while (0)
