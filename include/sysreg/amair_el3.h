...
static const union amair_el3 AMAIR_EL3_SAFEVAL =
{0
};



static inline union amair_el3 read_amair_el3( void )
{
    union amair_el3 tmp;
    __asm(
        "MRS %0, s3_6_c10_c3_0"
...
static inline void unsafe_write_amair_el3( union amair_el3 val )
{
    __asm(
        "MSR s3_6_c10_c3_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_amair_el3( ... ) \
    do \
    { \
        union amair_el3 tmp = AMAIR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_amair_el3(tmp); \
    } while (0)



#define read_modify_write_amair_el3( ... ) \
    do \
    { \
        union amair_el3 tmp = read_amair_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_amair_el3(tmp); \
    } while (0)
