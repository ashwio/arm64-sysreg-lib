...
static const union ttbr1_el2 TTBR1_EL2_SAFEVAL =
{0
};



static inline union ttbr1_el2 read_ttbr1_el2( void )
{
    union ttbr1_el2 tmp;
    __asm(
        "MRS %0, s3_4_c2_c0_1"
...
static inline void unsafe_write_ttbr1_el2( union ttbr1_el2 val )
{
    __asm(
        "MSR s3_4_c2_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_ttbr1_el2( ... ) \
    do \
    { \
        union ttbr1_el2 tmp = TTBR1_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr1_el2(tmp); \
    } while (0)



#define read_modify_write_ttbr1_el2( ... ) \
    do \
    { \
        union ttbr1_el2 tmp = read_ttbr1_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr1_el2(tmp); \
    } while (0)
