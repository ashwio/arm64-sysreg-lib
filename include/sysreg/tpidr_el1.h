...
static const union tpidr_el1 TPIDR_EL1_SAFEVAL =
{0
};



static inline union tpidr_el1 read_tpidr_el1( void )
{
    union tpidr_el1 tmp;
    __asm(
        "MRS %0, s3_0_c13_c0_4"
...
static inline void unsafe_write_tpidr_el1( union tpidr_el1 val )
{
    __asm(
        "MSR s3_0_c13_c0_4, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tpidr_el1( ... ) \
    do \
    { \
        union tpidr_el1 tmp = TPIDR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tpidr_el1(tmp); \
    } while (0)



#define read_modify_write_tpidr_el1( ... ) \
    do \
    { \
        union tpidr_el1 tmp = read_tpidr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tpidr_el1(tmp); \
    } while (0)
