...
static const union ttbr1_el1 TTBR1_EL1_SAFEVAL =
{0
};



static inline union ttbr1_el1 read_ttbr1_el1( void )
{
    union ttbr1_el1 tmp;
    __asm(
        "MRS %0, s3_0_c2_c0_1"
...
static inline void unsafe_write_ttbr1_el1( union ttbr1_el1 val )
{
    __asm(
        "MSR s3_0_c2_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_ttbr1_el1( ... ) \
    do \
    { \
        union ttbr1_el1 tmp = TTBR1_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr1_el1(tmp); \
    } while (0)



#define read_modify_write_ttbr1_el1( ... ) \
    do \
    { \
        union ttbr1_el1 tmp = read_ttbr1_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ttbr1_el1(tmp); \
    } while (0)
