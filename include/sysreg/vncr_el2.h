...
static const union vncr_el2 VNCR_EL2_SAFEVAL =
{0
};



static inline union vncr_el2 read_vncr_el2( void )
{
    union vncr_el2 tmp;
    __asm(
        "MRS %0, s3_4_c2_c2_0"
...
static inline void unsafe_write_vncr_el2( union vncr_el2 val )
{
    __asm(
        "MSR s3_4_c2_c2_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_vncr_el2( ... ) \
    do \
    { \
        union vncr_el2 tmp = VNCR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vncr_el2(tmp); \
    } while (0)



#define read_modify_write_vncr_el2( ... ) \
    do \
    { \
        union vncr_el2 tmp = read_vncr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_vncr_el2(tmp); \
    } while (0)
