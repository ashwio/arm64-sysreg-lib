...
static const union pmccntr_el0 PMCCNTR_EL0_SAFEVAL =
{0
};



static inline union pmccntr_el0 read_pmccntr_el0( void )
{
...
static inline void unsafe_write_pmccntr_el0( union pmccntr_el0 val )
{
    __asm(
        "MSR s3_3_c9_c13_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pmccntr_el0( ... ) \
    do \
    { \
        union pmccntr_el0 tmp = PMCCNTR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmccntr_el0(tmp); \
    } while (0)



#define read_modify_write_pmccntr_el0( ... ) \
    do \
    { \
        union pmccntr_el0 tmp = read_pmccntr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pmccntr_el0(tmp); \
    } while (0)
