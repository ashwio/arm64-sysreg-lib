...
static const union amair_el2 AMAIR_EL2_SAFEVAL =
{0
};



static inline union amair_el2 read_amair_el2( void )
{
    union amair_el2 tmp;
    __asm(
        "MRS %0, s3_4_c10_c3_0"
...
static inline void unsafe_write_amair_el2( union amair_el2 val )
{
    __asm(
        "MSR s3_4_c10_c3_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_amair_el2( ... ) \
    do \
    { \
        union amair_el2 tmp = AMAIR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_amair_el2(tmp); \
    } while (0)



#define read_modify_write_amair_el2( ... ) \
    do \
    { \
        union amair_el2 tmp = read_amair_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_amair_el2(tmp); \
    } while (0)
