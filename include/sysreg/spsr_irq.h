...
static const union spsr_irq SPSR_IRQ_SAFEVAL =
{0
};



static inline union spsr_irq read_spsr_irq( void )
{
    union spsr_irq tmp;
    __asm(
        "MRS %0, s3_4_c4_c3_0"
...
static inline void unsafe_write_spsr_irq( union spsr_irq val )
{
    __asm(
        "MSR s3_4_c4_c3_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_spsr_irq( ... ) \
    do \
    { \
        union spsr_irq tmp = SPSR_IRQ_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_irq(tmp); \
    } while (0)



#define read_modify_write_spsr_irq( ... ) \
    do \
    { \
        union spsr_irq tmp = read_spsr_irq(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsr_irq(tmp); \
    } while (0)
