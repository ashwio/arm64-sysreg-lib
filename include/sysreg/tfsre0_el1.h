...
static const union tfsre0_el1 TFSRE0_EL1_SAFEVAL =
{0
};



static inline union tfsre0_el1 read_tfsre0_el1( void )
{
...
static inline void unsafe_write_tfsre0_el1( union tfsre0_el1 val )
{
    __asm(
        "MSR s3_0_c5_c6_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_tfsre0_el1( ... ) \
    do \
    { \
        union tfsre0_el1 tmp = TFSRE0_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tfsre0_el1(tmp); \
    } while (0)



#define read_modify_write_tfsre0_el1( ... ) \
    do \
    { \
        union tfsre0_el1 tmp = read_tfsre0_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_tfsre0_el1(tmp); \
    } while (0)
