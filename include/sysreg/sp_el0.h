...
union sp_el0
{
    ...
    __extension__ struct {
        u64 Stack_pointer : 64;
    };
};



static const union sp_el0 SP_EL0_SAFEVAL =
{0
};



static inline union sp_el0 read_sp_el0( void )
{
    union sp_el0 tmp;
    __asm(
        "MRS %0, s3_0_c4_c1_0"
...
static inline void unsafe_write_sp_el0( union sp_el0 val )
{
    __asm(
        "MSR s3_0_c4_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_sp_el0( ... ) \
    do \
    { \
        union sp_el0 tmp = SP_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sp_el0(tmp); \
    } while (0)



#define read_modify_write_sp_el0( ... ) \
    do \
    { \
        union sp_el0 tmp = read_sp_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_sp_el0(tmp); \
    } while (0)
