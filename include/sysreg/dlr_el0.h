...
union dlr_el0
{
    ...
    __extension__ struct {
        u64 Restart_address : 64;
    };
};



static const union dlr_el0 DLR_EL0_SAFEVAL =
{0
};



static inline union dlr_el0 read_dlr_el0( void )
{
    union dlr_el0 tmp;
    __asm(
        "MRS %0, s3_3_c4_c5_1"
...
static inline void unsafe_write_dlr_el0( union dlr_el0 val )
{
    __asm(
        "MSR s3_3_c4_c5_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_dlr_el0( ... ) \
    do \
    { \
        union dlr_el0 tmp = DLR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dlr_el0(tmp); \
    } while (0)



#define read_modify_write_dlr_el0( ... ) \
    do \
    { \
        union dlr_el0 tmp = read_dlr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_dlr_el0(tmp); \
    } while (0)
