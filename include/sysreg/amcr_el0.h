...
static const union amcr_el0 AMCR_EL0_SAFEVAL =
{0
};



static inline union amcr_el0 read_amcr_el0( void )
{
    union amcr_el0 tmp;
    __asm(
        "MRS %0, s3_3_c13_c2_0"
...
static inline void unsafe_write_amcr_el0( union amcr_el0 val )
{
    __asm(
        "MSR s3_3_c13_c2_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_amcr_el0( ... ) \
    do \
    { \
        union amcr_el0 tmp = AMCR_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_amcr_el0(tmp); \
    } while (0)



#define read_modify_write_amcr_el0( ... ) \
    do \
    { \
        union amcr_el0 tmp = read_amcr_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_amcr_el0(tmp); \
    } while (0)
