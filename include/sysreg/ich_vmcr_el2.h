...
static const union ich_vmcr_el2 ICH_VMCR_EL2_SAFEVAL =
{0
};



static inline union ich_vmcr_el2 read_ich_vmcr_el2( void )
{
...
static inline void unsafe_write_ich_vmcr_el2( union ich_vmcr_el2 val )
{
    __asm(
...
#define safe_write_ich_vmcr_el2( ... ) \
    do \
    { \
        union ich_vmcr_el2 tmp = ICH_VMCR_EL2_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ich_vmcr_el2(tmp); \
    } while (0)



#define read_modify_write_ich_vmcr_el2( ... ) \
    do \
    { \
        union ich_vmcr_el2 tmp = read_ich_vmcr_el2(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_ich_vmcr_el2(tmp); \
    } while (0)
