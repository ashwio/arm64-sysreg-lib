...
union elr_el3
{
    ...
    __extension__ struct {
        u64 Return_address : 64;
    };
};



static const union elr_el3 ELR_EL3_SAFEVAL =
{0
};



static inline union elr_el3 read_elr_el3( void )
{
    union elr_el3 tmp;
    __asm(
        "MRS %0, s3_6_c4_c0_1"
...
static inline void unsafe_write_elr_el3( union elr_el3 val )
{
    __asm(
        "MSR s3_6_c4_c0_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_elr_el3( ... ) \
    do \
    { \
        union elr_el3 tmp = ELR_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_elr_el3(tmp); \
    } while (0)



#define read_modify_write_elr_el3( ... ) \
    do \
    { \
        union elr_el3 tmp = read_elr_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_elr_el3(tmp); \
    } while (0)
