...
static const union cntps_ctl_el1 CNTPS_CTL_EL1_SAFEVAL =
{0
};



static inline union cntps_ctl_el1 read_cntps_ctl_el1( void )
{
...
static inline void unsafe_write_cntps_ctl_el1( union cntps_ctl_el1 val )
{
    __asm(
        "MSR s3_7_c14_c2_1, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cntps_ctl_el1( ... ) \
    do \
    { \
        union cntps_ctl_el1 tmp = CNTPS_CTL_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntps_ctl_el1(tmp); \
    } while (0)



#define read_modify_write_cntps_ctl_el1( ... ) \
    do \
    { \
        union cntps_ctl_el1 tmp = read_cntps_ctl_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntps_ctl_el1(tmp); \
    } while (0)
