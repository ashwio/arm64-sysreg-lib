...
static const union spsel SPSEL_SAFEVAL =
{0
};



static inline union spsel read_spsel( void )
{
    union spsel tmp;
    __asm(
        "MRS %0, s3_0_c4_c2_0"
...
static inline void unsafe_write_spsel( union spsel val )
{
    __asm(
        "MSR s3_0_c4_c2_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_spsel( ... ) \
    do \
    { \
        union spsel tmp = SPSEL_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsel(tmp); \
    } while (0)



#define read_modify_write_spsel( ... ) \
    do \
    { \
        union spsel tmp = read_spsel(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_spsel(tmp); \
    } while (0)
