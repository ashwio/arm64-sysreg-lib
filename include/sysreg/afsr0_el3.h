...
static const union afsr0_el3 AFSR0_EL3_SAFEVAL =
{0
};



static inline union afsr0_el3 read_afsr0_el3( void )
{
    union afsr0_el3 tmp;
    __asm(
        "MRS %0, s3_6_c5_c1_0"
...
static inline void unsafe_write_afsr0_el3( union afsr0_el3 val )
{
    __asm(
        "MSR s3_6_c5_c1_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_afsr0_el3( ... ) \
    do \
    { \
        union afsr0_el3 tmp = AFSR0_EL3_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_afsr0_el3(tmp); \
    } while (0)



#define read_modify_write_afsr0_el3( ... ) \
    do \
    { \
        union afsr0_el3 tmp = read_afsr0_el3(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_afsr0_el3(tmp); \
    } while (0)
