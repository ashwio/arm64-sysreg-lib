...
static const union cntfrq_el0 CNTFRQ_EL0_SAFEVAL =
{0
};



static inline union cntfrq_el0 read_cntfrq_el0( void )
{
...
static inline void unsafe_write_cntfrq_el0( union cntfrq_el0 val )
{
    __asm(
        "MSR s3_3_c14_c0_0, %0"
        : /**/ : "r" (val._)
...
#define safe_write_cntfrq_el0( ... ) \
    do \
    { \
        union cntfrq_el0 tmp = CNTFRQ_EL0_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntfrq_el0(tmp); \
    } while (0)



#define read_modify_write_cntfrq_el0( ... ) \
    do \
    { \
        union cntfrq_el0 tmp = read_cntfrq_el0(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_cntfrq_el0(tmp); \
    } while (0)
