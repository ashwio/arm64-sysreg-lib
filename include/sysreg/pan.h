...
static const union pan PAN_SAFEVAL =
{0
};



static inline union pan read_pan( void )
{
    union pan tmp;
    __asm(
        "MRS %0, s3_0_c4_c2_3"
...
static inline void unsafe_write_pan( union pan val )
{
    __asm(
        "MSR s3_0_c4_c2_3, %0"
        : /**/ : "r" (val._)
...
#define safe_write_pan( ... ) \
    do \
    { \
        union pan tmp = PAN_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pan(tmp); \
    } while (0)



#define read_modify_write_pan( ... ) \
    do \
    { \
        union pan tmp = read_pan(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_pan(tmp); \
    } while (0)
