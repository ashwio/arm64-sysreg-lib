...
static const union mdscr_el1 MDSCR_EL1_SAFEVAL =
{0
};



static inline union mdscr_el1 read_mdscr_el1( void )
{
    union mdscr_el1 tmp;
    __asm(
        "MRS %0, s2_0_c0_c2_2"
...
static inline void unsafe_write_mdscr_el1( union mdscr_el1 val )
{
    __asm(
        "MSR s2_0_c0_c2_2, %0"
        : /**/ : "r" (val._)
...
#define safe_write_mdscr_el1( ... ) \
    do \
    { \
        union mdscr_el1 tmp = MDSCR_EL1_SAFEVAL; \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_mdscr_el1(tmp); \
    } while (0)



#define read_modify_write_mdscr_el1( ... ) \
    do \
    { \
        union mdscr_el1 tmp = read_mdscr_el1(); \
        ARM64_SYSREG_CALL_FOR_EACH(ARM64_SYSREG_SET, __VA_ARGS__) \
        unsafe_write_mdscr_el1(tmp); \
    } while (0)
